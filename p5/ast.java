...
class FnBodyNode extends ASTnode {
    ...
    public void typeCheck(TypeNode fnType) {
...
class ExpListNode extends ASTnode {
    ...
    public Type typeCheck(List<Type> paramsTypes) {
        boolean hasError = false;
        Iterator<ExpNode> itActual = myExps.iterator();
        Iterator<Type> itFormal = paramsTypes.iterator();
        while (itActual.hasNext()) {
            ExpNode actualExp = itActual.next();
            Type actualExpType = actualExp.typeCheck();
            if (actualExpType.isErrorType()) {return new ErrorType();}
            if (!actualExpType.equals(itFormal.next())) {
                ErrMsg.fatal(actualExp.lineNum(), actualExp.charNum(), "Type of actual does not match type of formal");
                hasError = true;
            }
        }
        if (hasError) {
...
abstract class StmtNode extends ASTnode {
    ...
    abstract public void typeCheck(TypeNode fnType);
}
...
class PostIncStmtNode extends StmtNode {
    ...
    public void typeCheck(TypeNode fnType) {
        if (myExp instanceof IdNode) { // if loc is simply an id
            if (! ((IdNode)myExp).sym().getType().isIntType()){ // not int, report error
                ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Arithmetic operator applied to non-numeric operand");
            }
        } else { // loc is an expression of dot access
    ...
    public void typeCheck(TypeNode fnType) {
        ...
        if (myExp instanceof CallExpNode) {
            ...
            if (type.isFnType()) {
                ...
                ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Attempt to write a stract variable");
                return;
            } else if (type.isStructDefType()) {
...
class WhileStmtNode extends StmtNode {
    ...
    public void typeCheck(TypeNode fnType) {
        ...
        if (!expType.isBoolType()) {
            if (!expType.isErrorType()) {
                ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Non-bool expression used as an while condition");
            }
        }
        myStmtList.typeCheck(fnType);
    }
...
class ReturnStmtNode extends StmtNode {
    ...
    public void typeCheck(TypeNode fnType) {
        if (myExp == null) {
            if (!fnType.type().isVoidType()) {
                ErrMsg.fatal(0,0, "Missing return value");
            }
        } else {
            Type retType = myExp.typeCheck();
            if (retType.isErrorType()) {return;}
            if (fnType.type().isVoidType()) {
                ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Return with a value in a void function");
            } else if (!fnType.type().equals(retType)) {
...
class AssignNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        lType = myLhs.typeCheck();
        rType = myExp.typeCheck();
        
        if (lType.equals(rType)) {
            if ((myLhs instanceof IdNode) && (myExp instanceof IdNode)) {
                if ((((IdNode)myLhs).sym().getType().isFnType()) && (((IdNode)myExp).sym().getType().isFnType())) {
                    ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(), "Function assignment");
                    return new ErrorType();
                }
                if ((((IdNode)myLhs).sym().getType().isStructType()) && (((IdNode)myExp).sym().getType().isStructType())) {
                    ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(), "Struct variable assignment");
                    return new ErrorType();
                }
                if ((((IdNode)myLhs).sym().getType().isStructDefType()) && (((IdNode)myExp).sym().getType().isStructDefType())) {
            ...
            if (myExp instanceof CallExpNode) {
                IdNode funName = ((CallExpNode)myExp).getFunId();
                if (funName.sym().getType().isFnType()) {
...
class CallExpNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        // check if number of paramters match
        FnSym fun = (FnSym)myId.sym();
        if ((myExpList == null) || (myExpList.getExpListSize() == 0) ) {
        ...
        int numActuals = myExpList.getExpListSize();
        if (numActuals != fun.getNumParams()) {
            ErrMsg.fatal(myId.lineNum(), myId.charNum(), "Function call with wrong number of args");
            return new ErrorType();
        }
        // number matches
        // check if types of parameters match respectively
        if (myExpList.typeCheck(fun.getParamTypes()).isErrorType()) {
            return new ErrorType();
        } else {
            return fun.getReturnType();
        }
        // the return value should be the retureType of function called
    }
...
abstract class BinaryExpNode extends ExpNode {
    ...
    public Type typeCheck() {
        Type lType, rType;
        lType = myExp1.typeCheck();
        rType = myExp2.typeCheck();
...
class UnaryMinusNode extends UnaryExpNode {
    ...
    public Type typeCheck() {
        Type type = myExp.typeCheck();
        if (type.isErrorType()) {return new ErrorType();}
        if (type.isIntType()) {
            return type;
        } else {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(), "Arithmetic operator applied to non-numeric operand");
...
class AndNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if (typeOne.isBoolType()) {
            if (typeTwo.isBoolType()) {
                return typeOne;
            } else if (typeTwo.isErrorType()) {
                return new ErrorType();
            } else {
...
class EqualsNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if ((typeOne.isErrorType()) || (typeTwo.isErrorType())) {
            return new ErrorType();
        } 

        if (typeOne.equals(typeTwo)) {
            if ((myExp1 instanceof IdNode) && (myExp2 instanceof IdNode)) {
                if ((((IdNode)myExp1).sym().getType().isFnType()) && (((IdNode)myExp2).sym().getType().isFnType())) {
                    ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(), "Equality operator applied to functions");
                    return new ErrorType();
                }
                if ((((IdNode)myExp1).sym().getType().isStructType()) && (((IdNode)myExp2).sym().getType().isStructType())) {
                    ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(), "Equality operator applied to struct variables");
                    return new ErrorType();
                }
                if ((((IdNode)myExp1).sym().getType().isStructDefType()) && (((IdNode)myExp2).sym().getType().isStructDefType())) {
                    ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(), "Equality operator applied to struct name");
                    return new ErrorType();
                }
            }
            if (typeOne.isVoidType()) {
