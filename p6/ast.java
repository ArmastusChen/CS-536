...
class ProgramNode extends ASTnode {
    ...
    public void nameAnalysis() {
        ...
        myDeclList.nameAnalysis(symTab, false, 0);
    }
    
    public boolean hasMain() {
        return myDeclList.hasMain();
    }
...
class DeclListNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, boolean isLocalVar, int numVar) {
        nameAnalysis(symTab, symTab, isLocalVar, numVar);
    }
     ...
     * decls in the list.
     */    
    public void nameAnalysis(SymTable symTab, SymTable globalTab, boolean isLocalVar, int numVar) {
        int os = 0;
        if (isLocalVar) {
            os = -4;
        }
        for (DeclNode node : myDecls) {
            if (node instanceof VarDeclNode) {
                ...
                if (isLocalVar) {
                    sym.setIsLocalVar(true);
                    os = os - 4;
                    sym.setOffset(os);
                } else {
                    sym.setIsLocalVar(false);
                }
            } else {
    ...
    // method that check if main appeared in the declaration list
    public boolean hasMain() {
        for (DeclNode node : myDecls) {
            if (node instanceof FnDeclNode) {
                if (((FnDeclNode)node).hasMain()) {
                    return true;
                }
            }
        }
...
class FnBodyNode extends ASTnode {
    ...
    public void nameAnalysis(SymTable symTab) {
        ...
        int numVarInDecl = myDeclList.getNumVarDecl();
        // System.out.println("number in line 322: " + numVarInDecl);
        myStmtList.nameAnalysis(symTab, numVarInDecl);
        int numVarInStmt = myStmtList.getNumVarDecl();
        this.numVarInBody = numVarInDecl + numVarInStmt;
    }

    public int getNumVarInBody() {
        return numVarInBody;
    }    
    ...
    public void codeGen(String exitLabel) {
        Codegen.p.println("\t\t#Begin Function Body");
        // no need to generate any code for the declarations, just call the 
        // code generation function of StmtListNode
    ...
    public void unparse(PrintWriter p, int indent) {
        ...
        p.println("total num of locals in FnBody: " + numVarInBody);
    }

    // 2 kids
    private DeclListNode myDeclList;
    private StmtListNode myStmtList;
    private int numVarInBody;
}
...
class StmtListNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, int numVar) {
        for (StmtNode node : myStmts) {
            if (node instanceof IfStmtNode) {
                ((IfStmtNode)node).nameAnalysis(symTab, numVar);
                numVar = numVar + ((IfStmtNode)node).getNumVarDecl();
            } else if (node instanceof IfElseStmtNode) {
                ((IfElseStmtNode)node).nameAnalysis(symTab, numVar);
                numVar = numVar + ((IfElseStmtNode)node).getNumVarDecl();
            } else if (node instanceof WhileStmtNode) {
                ((WhileStmtNode)node).nameAnalysis(symTab, numVar);
                numVar = numVar + ((WhileStmtNode)node).getNumVarDecl();
            } else {
                node.nameAnalysis(symTab, numVar);
            }            
        }
    }    
    ...
    public int getNumVarDecl() {
        ...
        for (StmtNode node:myStmts) {
            int toAdd = 0;
            if (node instanceof IfStmtNode) {
                toAdd = ((IfStmtNode)node).getNumVarDecl();
            }
            if (node instanceof IfElseStmtNode) {
                toAdd = ((IfElseStmtNode)node).getNumVarDecl();
            }
            if (node instanceof WhileStmtNode) {
                toAdd = ((WhileStmtNode)node).getNumVarDecl();
            }
            numVarInStmt = numVarInStmt + toAdd;
        }
        return numVarInStmt;
    }
...
class FnDeclNode extends DeclNode {
    ...
    public SemSym nameAnalysis(SymTable symTab) {
        ...
        // set the field: number of total local variables in function body
        sym.setNumLocalVar(myBody.getNumVarInBody());
    ...
    // function that check if main has been declared
    public boolean hasMain() {
        return this.hasMain;
    }  
    /**
    ...
    public void codeGen() {
        ...
        if (name.equals("main")) {
            ...
            // Otherfunction preamble
            
            name = "_" + name;
            Codegen.genLabel(name);
        }
        // 2. function prologue
        Codegen.p.println("\t\t#Begin Function Prologue");
        // 2.1. save the return address
        Codegen.genPush(Codegen.RA);
        // 2.2. push the control link
        Codegen.genPush(Codegen.FP);
        // 2.3. make space for locals
        int localSpace = 4 * myBody.getNumVarInBody();
        if (localSpace != 0) {
            Codegen.generate("subu", Codegen.SP, Codegen.SP, localSpace);
        }
        // 2.4. update the frame pointer
        Codegen.generate("addu", Codegen.FP, Codegen.SP, localSpace+8);
        // 3. function body
        String exitLabel = Codegen.nextLabel();
        myBody.codeGen(exitLabel); // the label should be parsed to all return stmt
        // generate the "return" code just once after generating the code
        // for the function body.
        int paramSpace = 4 * myFormalsList.length();
        //Codegen.generateIndexed("lw", Codegen.V0, Codegen.FP, -paramSpace);
        Codegen.genLabel(exitLabel);
        // 4. function epilogue
        Codegen.p.println("\t\t#Begin Function Epilogue");
        // 4.1. Restore return address
        Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP, 0);
        // 4.2. Restore the frame pointer
        Codegen.generate("move", Codegen.T0, Codegen.FP);
        Codegen.generateIndexed("lw", Codegen.FP, Codegen.FP, -4);
        // 4.3. Restore the stack pointer
        ...
        if (name.equals("main")) {
            ...
            // 4.4. Jump to the return address
...
class StructDeclNode extends DeclNode {
    ...
    public SemSym nameAnalysis(SymTable symTab) {
        ...
        myDeclList.nameAnalysis(structSymTab, symTab, false, 0);
...
abstract class StmtNode extends ASTnode {
    abstract public void nameAnalysis(SymTable symTab, int numVar);
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        Codegen.generate("add", "$t2", "$t2", Codegen.T1);
        ((IdNode)myExp).genAddr();
        Codegen.genPop(Codegen.T1);
        Codegen.generateIndexed("sw", "$t2", Codegen.T1, 0);
        Codegen.genPush("$t2");
    }
...
class PostDecStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        Codegen.genPop("$t2");
        Codegen.generate("li", Codegen.T1, 1);
        Codegen.generate("sub", "$t2", "$t2", Codegen.T1);
        ((IdNode)myExp).genAddr();
        Codegen.genPop(Codegen.T1);
        Codegen.generateIndexed("sw", "$t2", Codegen.T1, 0);
        Codegen.genPush("$t2");
...
class WriteStmtNode extends StmtNode {
    ...
    public void typeCheck(Type retType) {
        ...
        if (myExpType.isStructDefType()) {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(),
                         "Attempt to write a struct name");
        }
        
        if (myExpType.isStructType()) {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(),
                         "Attempt to write a struct variable");
        }
        
        if (myExpType.isVoidType()) {
            ErrMsg.fatal(myExp.lineNum(), myExp.charNum(),
...
class IfStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab, int numVar) {
        myExp.nameAnalysis(symTab);
        symTab.addScope();
        myDeclList.nameAnalysis(symTab, true, numVar);
        numVar = numVar + myDeclList.getNumVarDecl();
        myStmtList.nameAnalysis(symTab, numVar);
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
            System.err.println("Unexpected EmptySymTableException " +
    ...
    public int getNumVarDecl() {
        return myDeclList.getNumVarDecl() + myStmtList.getNumVarDecl();
    }
...
class IfElseStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab, int numVar) {
        myExp.nameAnalysis(symTab);
        symTab.addScope();
        myThenDeclList.nameAnalysis(symTab, true, numVar);
        numVar = numVar + myThenDeclList.getNumVarDecl();
        myThenStmtList.nameAnalysis(symTab, numVar);
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
            System.err.println("Unexpected EmptySymTableException " +
        ...
        symTab.addScope();
        numVar = numVar + myThenStmtList.getNumVarDecl();
        myElseDeclList.nameAnalysis(symTab, true, numVar);
        numVar = numVar + myElseDeclList.getNumVarDecl();
        myElseStmtList.nameAnalysis(symTab, numVar);
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
            System.err.println("Unexpected EmptySymTableException " +
    ...
    public int getNumVarDecl() {
        return myThenDeclList.getNumVarDecl() 
             + myElseDeclList.getNumVarDecl()  
             + myThenStmtList.getNumVarDecl() 
             + myElseStmtList.getNumVarDecl();
    }
...
class WhileStmtNode extends StmtNode {
    ...
    public void codeGen(String exitLabel) {
        String beforeWhileLabel = Codegen.nextLabel();
        String doneLabel = Codegen.nextLabel();
        Codegen.genLabel(beforeWhileLabel);
        myExp.codeGen();
        Codegen.genPop(Codegen.T0);
        Codegen.generate("beq", Codegen.T0, Codegen.FALSE, doneLabel);
        myStmtList.codeGen(exitLabel);
        Codegen.generate("j ", beforeWhileLabel);
        Codegen.genLabel(doneLabel);
    }
...
class IdNode extends ExpNode {
    ...
    // Three different code generation method
    // genJumpAndLink
    public void genJumpAndLink(String funLabel) {
        String temp = "_" + funLabel;
        Codegen.generate("jal ", temp);
        //Codegen.genPush(Codegen.V0);
    }

    // general code generation method for all exp node
    public void codeGen() {
        // treat local and global variable differently
        if (this.mySym.isLocalVar()) {
            // for local variable, get its offset and load it into $t0
            int offset = this.mySym.getOffset();
            Codegen.generateIndexed("lw", Codegen.T0, Codegen.FP, offset);
        } else {
            // for global variable, get its offset and load it into $t0
            String tempName = "_" + myStrVal;
            Codegen.generate("lw", Codegen.T0, tempName);
        }  
        Codegen.genPush(Codegen.T0);
    }
    ...
    public void genAddr() {
        ...
        if (this.mySym.isLocalVar()) {
            ...
            // for global variable, get its offset and load it into $t0
            String tempName = "_" + myStrVal;
            Codegen.generate("la", Codegen.T0, tempName);
        }
        Codegen.genPush(Codegen.T0);
    }
...
class AssignNode extends ExpNode {
    ...
    public void codeGen() {
        // evaluate RHS and leaving the value on the stack
        myExp.codeGen();
        // push the address of LHS ID onto the stack
...
class CallExpNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        if(myExpList != null){
			...
			myExpList.codeGen(); // the pushing is done by each exp node
			
		}
        // Jump and link: jump to the called function, leaving the return
        // address in the RA register
        String funName = myId.name();
        ((IdNode)myId).genJumpAndLink(funName);
        
        if(myExpList != null){
			
			Codegen.generate("addu", Codegen.SP, Codegen.SP, myExpList.size() * 4); // the pushing is done by each exp node
		}	
...
class UnaryMinusNode extends UnaryExpNode {
    ...
    public void codeGen() {
        if (myExp instanceof IntLitNode) {
            Codegen.generate("li", Codegen.T0, -((IntLitNode)myExp).getIntVar());
        } else {
            myExp.codeGen();
            Codegen.genPop(Codegen.T0);
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        myExp1.codeGen();
        Codegen.genPop(Codegen.T0);
        // if the LHS exp is true, do evaluation on the RHS;
...
class EqualsNode extends EqualityExpNode {
    ...
    public void codeGen() {
        // deal with string casese separately
...
class LessNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.generate("bltz", Codegen.T0, trueLabel);
        Codegen.generate("li", Codegen.T0, Codegen.FALSE);
        Codegen.genPush(Codegen.T0);
        Codegen.generate("j ", doneLabel);
        Codegen.genLabel(trueLabel);
...
class GreaterNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.genPop(Codegen.T0);
        Codegen.generate("sub", Codegen.T0, Codegen.T0, Codegen.T1);
        Codegen.generate("bgtz", Codegen.T0, trueLabel);
        Codegen.generate("li", Codegen.T0, Codegen.FALSE);
        Codegen.genPush(Codegen.T0);
        Codegen.generate("j ", doneLabel);
        Codegen.genLabel(trueLabel);
...
class LessEqNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.genPop(Codegen.T0);
        Codegen.generate("sub", Codegen.T0, Codegen.T0, Codegen.T1);
        Codegen.generate("blez", Codegen.T0, trueLabel);
        Codegen.generate("li", Codegen.T0, Codegen.FALSE);
        Codegen.genPush(Codegen.T0);
        Codegen.generate("j ", doneLabel);
        Codegen.genLabel(trueLabel);
