...
int funC (bool boolB, int intA, bool boolC) {
	...
	while (funA() == funA(intY, intZ)) { // 61:9 Equality operator applied to void functions  61:9 Function call with wrong number of args  61:30 Type of actual does not match type of formal
		...
		while ((intA + 35) > boolA) { // 74:24 Relational operator applied to non-numeric operand
			...
			return insider.SSbool; // 77:11 Bad return value
...
void funD (int intA, int intB, bool boolC) {
	...
	int intD;
	bool boolE;
	boolC = funC(2-"a"/boolC, boolE, intD); // 96:2 Type mismatch   96:17 Arithmetic operator applied to non-numeric operand    96:21 Arithmetic operator applied to non-numeric operand   96:28 Type of actual does not match type of formal  96:35 Type of actual does not match type of formal
	...
	if (-1) { // 100:7 Non-bool expression used as an if condition
		return -boolC; //  	101:11  Return with a value in a void function    101:11   Arithmetic operator applied to non-numeric operand
		while(structInD.structA.Sint && (!boolE>=1) || structInD.structAA.SSbool) { // 102:27 Logical operator applied to non-bool operand  102:37 Relational operator applied to non-numeric operand    
